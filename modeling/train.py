...
class TrainingSession(object):
    ...
    def __init__(self,
        ...
        self.sg = StyleGAN(latent_size=self.config['latent_size'],
        ...
        if self.steps == 0:
            self.gen = self.sg.build_gen()
            self.dis = self.sg.build_dis()
            self.save(0)
        else:
        ...
        self.gen_eval = self.sg.build_gen_eval(self.map, self.gen)
        ...
        self.n_layers = self.sg.n_blocks * 2

    def train(self):

        if random() < mixed_prob:
            style = self.sg.mixed_latent_list(self.batch_size)
        else:
            style = self.sg.random_latent_list(self.batch_size)
    ...
    def train_step(self, images, latents, perform_gp=True, perform_pl=False):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            noise = self.sg.random_noise(self.batch_size)
    ...
    def evaluate(self, num=0, trunc=1.0):
        ...
        n1 = self.sg.random_latent_list(64)
        n2 = self.sg.random_noise(64)
